...
CREATE TABLE `employee` (
  ...
  `adminRights` integer(1) DEFAULT 0 COMMENT 'bypasses everything, even the product constraints',
  `i_id` integer
);
...
CREATE TABLE `appointment` (
  ...
  `p_id` integer,
  `appointmentDate` datetime COMMENT 'when appointment was scheduled to be'
);
